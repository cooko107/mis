...
class Nku_Plan(models.Model):
    ...
    items = fields.One2many('jm.nku.plan.items', 'plan_con', u'收费列表')
    ...
    def get_name(self):
        if(self.major == '1'):
            self.env.cr.execute("UPDATE jm_nku_plan set show_name='%s' where name='%s' and batch='%s' and level='%s' and study_center='%s'"
    ...
    def delete(self, cr, uid, context):
        ...
        for id in context.get('active_ids'):
            cr.execute("select bm_fee,tuition,jc_fee,ptfw_fee,tuition2,jc_fee2,ptfw_fee2,lwzd_fee,dzsx_fee"
                       ",qcfd_fee,qcfd_fee2,lwcx_fee from jm_nku_plan where id=%s" % id)
            items = cr.fetchall()
            print items
            i = 0
            for item in items[0]:
                ...
                pro = cr.fetchall()[0][0]
                cr.execute("insert into jm_nku_plan_items (item,money,plan_con) values(%s, %s, %s)" % (pro, item, id))
                i += 1

    '''@api.multi
    @api.depends('batch', 'level')
    def name_get(self):
        return [(r.id, '%s'%(r.name+r.batch))for r in self]'''


class Nku_Plan_items(models.Model):
    _name = 'jm.nku.plan.items'

    item = fields.Many2one('jm.fee.project',u'收费项目')
    money = fields.Float(u'收费金额', digits=(7,0))
    plan_con = fields.Many2one('jm.nku.plan', ondelete='cascade')