...
class Tju_Report(models.Model):
    ...
    def generate_report(self,cr,uid,ids,context=None):
        ...
        if(starttime > endtime or not starttime or not endtime):
            ...
            cr.execute("delete from jm_tju_report WHERE 1=1")
            #选择高起专各个学习中心的id
            cr.execute("SELECT id,name from jm_tju_plan where batch='%s' and type='高起专' and study_center='1'" %(batch.name))
            gqz = cr.fetchall()
            #选择专升本各个学习中心的id
            cr.execute("SELECT id,name from jm_tju_plan where batch='%s' and type='专升本' and study_center='1'" %(batch.name))
            zsb = cr.fetchall()
            #遍历各个高起专的学习中心
            for each_gqz in gqz:
                each_sum = 0
                # 统计改高起专的学习中心报名人数
                cr.execute("SELECT COUNT(id) from jm_tju_student where study_center=%s and entry_date between "
                           "'%s' and '%s'" %(each_gqz[0], starttime, endtime))
                eachgqz_count = cr.fetchall()
                #写入报表数据库
                cr.execute("INSERT into jm_tju_report(batch,start_time,end_time,study_center, zhuan_count) values(%s,'%s','%s','%s', %s)"
            ...
            for each_zsb in zsb:
                ...
                #查看改学习中心是否存在
                cr.execute("SELECT zhuan_count from jm_tju_report where study_center='%s' and start_time='%s' and end_time='%s'"
                           "" % (each_zsb[1],starttime, endtime))
                zhuan_count = cr.fetchall()
                #存在 则更新
                if (zhuan_count):
                    cr.execute("UPDATE jm_tju_report set (ben_count,sum_count)=(%s,%s) where study_center='%s' and start_time='%s' and end_time='%s'"
                               %(eachzsb_count[0][0],eachzsb_count[0][0]+zhuan_count[0][0],each_zsb[1], starttime, endtime))
                #否则 插入
                else:
                    cr.execute("INSERT into jm_tju_report(batch,start_time,end_time,study_center, zhuan_count,ben_count,sum_count) values(%s,'%s','%s',%s,0, %s,%s)"
                           %(batch.id,starttime,endtime,each_zsb[1], eachzsb_count[0][0],eachzsb_count[0][0]))
            #将无效数据删除
            cr.execute("DELETE from jm_tju_report where study_center is NULL")
            return {
