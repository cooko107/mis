...
class Zzu_Report(models.Model):
    ...
    def generate_report2(self,cr,uid,ids,context=None):
        ...
        if(starttime > endtime or not starttime or not endtime):
            ...
            cr.execute("delete from jm_zzu_report2 WHERE 1=1")
            #选择高起专各个渠道的id
            cr.execute("SELECT id,call from jm_custom")
            all_qd = cr.fetchall()

            cr.execute("select id from jm_zzu_plan where batch='%s' and name='高起专' and type='1'" %(batch.name))
            gqz = cr.fetchall()

            cr.execute("select id from jm_zzu_plan where batch='%s' and name='专升本' and type='1'" %(batch.name))
            zsb = cr.fetchall()
            ...
            for each_gqz in gqz:
                for each_qd in all_qd:
                    each_sum = 0
                # 统计改高起专的学习中心报名人数
                    cr.execute("SELECT COUNT(id) from jm_zzu_student where  level=%s and inputer_dpt=%s and entry_date between "
                                "'%s' and '%s'" %(each_gqz[0],each_qd[0],starttime, endtime))
                    eachgqz_count = cr.fetchall()
                #写入报表数据库

                    cr.execute("INSERT into jm_zzu_report2(batch,start_time,end_time,qd, zhuan_count) values(%s,'%s','%s','%s', %s)"
            ...
            for each_zsb in zsb:
                ...
                #查看改学习中心是否存在
                    cr.execute("SELECT zhuan_count from jm_zzu_report2 where qd='%s' and start_time='%s' and end_time='%s'"
                                "" % (each_qd[1],starttime, endtime))
                    zhuan_count = cr.fetchall()
                #存在 则更新
                    if (zhuan_count):
                        cr.execute("UPDATE jm_zzu_report2 set (ben_count,sum_count)=(%s,%s) where qd='%s' and start_time='%s' and end_time='%s'"
                                     %(eachzsb_count[0][0],eachzsb_count[0][0]+zhuan_count[0][0],each_qd[1], starttime, endtime))
                #否则 插入
                    else:
                        cr.execute("INSERT into jm_zzu_report2(batch,start_time,end_time,qd, zhuan_count,ben_count,sum_count) values(%s,'%s','%s','%s',0, %s,%s)"
                                %(batch.id,starttime,endtime,each_qd[1], eachzsb_count[0][0],eachzsb_count[0][0]))
            #将无效数据删除
            cr.execute("DELETE from jm_zzu_report2 where qd is NULL")
            return {
