...
class Nku_Plan_Generate(models.Model):
    _name = 'jm.nku.plan.generate'
    ...
    items = fields.One2many('jm.nku.plan.generate.items', 'plan_con', u'收费')

    @api.multi
    def generate(self):
        batch = self.batch.name
        level = self.level.name
        if not self.env['jm.nku.plan'].search([('name','=',batch),('batch','=','1')]):
            self.env['jm.nku.plan'].create({
                'name':batch,
                'batch':'1',
            })
        if not self.env['jm.nku.plan'].search([('name','=',level),('batch','=',batch),('level','=','1')]):
            self.env['jm.nku.plan'].create({
                'name':level,
                'batch':batch,
                'level':'1',
        ...
        for sc in self.sc:
            ...
            if not self.env['jm.nku.plan'].search(domains):
                self.env['jm.nku.plan'].create({
                    'name':sc.name,
                    'batch':batch,
                    'level':level,
            ...
            for major in self.major:
                ...
                if not self.env['jm.nku.plan'].search(domains):
                    new_plan = self.env['jm.nku.plan'].create({
                        'name':major.name,
                        'batch':batch,
                        'level':level,
                        'study_center':sc.name,
                    ...
                    for item in self.items:
                        self.env['jm.nku.plan.items'].create({
                            'item':item.item.id,
                            'money':item.money,
                            'plan_con':new_plan.id,
                        })
                else:
                    old_plan = self.env['jm.nku.plan'].search(domains)
        ...
        self.env.cr.execute("DELETE from jm_nku_plan_generate where 1=1")
        self.env.cr.execute("DELETE from jm_nku_plan_generate_items where 1=1")
        return {
...
class Nku_plan_generate_items(models.Model):
    _name = 'jm.nku.plan.generate.items'

    item = fields.Many2one('jm.fee.project',u'收费项目')
    money = fields.Float(u'收费金额', digits=(7,0))
    plan_con = fields.Many2one('jm.nku.plan.generate', ondelete='cascade')