...
class Tju_Plan(models.Model):
    _name = 'jm.tju.plan'

    _order = 'batch desc, type desc, study_center, major'


    name = fields.Char(u'名称')
    batch = fields.Char(u'批次')
    type = fields.Char(u'层次')
    school = fields.Char(u'学校')
    major = fields.Char(u'是否专业')
    study_center = fields.Char(u'学习中心')

    items = fields.One2many('jm.tju.plan.items', 'plan_con', u'收费列表')
    ...
    def get_name(self):
        if(self.major == '1'):
            self.env.cr.execute("UPDATE jm_tju_plan set show_name='%s' where name='%s' and batch='%s' and type='%s' and study_center='%s'"
                               %(self.name, self.name, self.batch, self.type, self.study_center))
    ...
    def delete(self, cr, uid, context):
        ...
        for id in context.get('active_ids'):
            cr.execute("select bm_fee,tuition,jc_fee,ptfw_fee,tuition2,jc_fee2,ptfw_fee2,lwzd_fee,dzsx_fee"
                       ",qcfd_fee,qcfd_fee2,lwcx_fee from jm_tju_plan where id=%s" % id)
            items = cr.fetchall()
            print items
            i = 0
            for item in items[0]:
                ...
                pro = cr.fetchall()[0][0]
                cr.execute("insert into jm_tju_plan_items (item,money,plan_con) values(%s, %s, %s)" %(pro, item, id))
                i += 1

    '''@api.multi
    @api.depends('batch', 'type')
    def name_get(self):
        return [(r.id, '%s'%(r.name+r.batch))for r in self]'''


class Tju_Plan_items(models.Model):
    _name = 'jm.tju.plan.items'
