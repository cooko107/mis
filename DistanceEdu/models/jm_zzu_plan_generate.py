...
class Zzu_Plan_Generate(models.Model):
    _name = 'jm.zzu.plan.generate'
    ...
    items = fields.One2many('jm.zzu.plan.generate.items', 'plan_con', u'收费')

    @api.multi
    def generate_plan(self):
        batch = self.batch.name
        level = self.level.name
        if not self.env['jm.zzu.plan'].search([('name', '=', batch), ('batch', '=', '1')]):
            self.env['jm.zzu.plan'].create({
                'name': batch,
                'batch': '1',
            })
        if not self.env['jm.zzu.plan'].search([('name', '=', level), ('batch', '=', batch), ('type', '=', '1')]):
            self.env['jm.zzu.plan'].create({
                'name': level,
                'batch': batch,
                'type': '1',
        ...
        for sc in self.sc:
            ...
            if not self.env['jm.zzu.plan'].search(domains):
                self.env['jm.zzu.plan'].create({
                    'name': sc.name,
                    'batch': batch,
                    'type': level,
            ...
            for major in self.major:
                ...
                if not self.env['jm.zzu.plan'].search(domains):
                    new_plan = self.env['jm.zzu.plan'].create({
                        'name': major.name,
                        'batch': batch,
                        'type': level,
                        'study_center': sc.name,
                    ...
                    for item in self.items:
                        self.env['jm.zzu.plan.items'].create({
                            'item': item.item.id,
                            'money': item.money,
                            'plan_con': new_plan.id,
                        })
                else:
                    old_plan = self.env['jm.zzu.plan'].search(domains)
        ...
        self.env.cr.execute("DELETE from jm_zzu_plan_generate where 1=1")
        self.env.cr.execute("DELETE from jm_zzu_plan_generate_items where 1=1")
        return {
...
class zzu_plan_generate_items(models.Model):
    _name = 'jm.zzu.plan.generate.items'

    item = fields.Many2one('jm.fee.project', u'收费项目')
    money = fields.Float(u'收费金额', digits=(7, 0))
    plan_con = fields.Many2one('jm.zzu.plan.generate', ondelete='cascade')