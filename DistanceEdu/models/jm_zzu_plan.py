...
class Zzu_Plan(models.Model):
    ...
    items = fields.One2many('jm.zzu.plan.items', 'plan_con', u'收费列表')
    ...
    def get_name(self):
        if(self.major == '1'):
            self.env.cr.execute("UPDATE jm_zzu_plan set show_name='%s' where name='%s' and batch='%s' and type='%s' and study_center='%s'"
    ...
    def delete(self, cr, uid, context):
        ...
        for id in context.get('active_ids'):
            cr.execute("select bm_fee,tuition,jc_fee,ptfw_fee,tuition2,jc_fee2,ptfw_fee2,lwzd_fee,dzsx_fee"
                       ",qcfd_fee,qcfd_fee2,lwcx_fee from jm_zzu_plan where id=%s" % id)
            items = cr.fetchall()
            print items
            i = 0
            for item in items[0]:
                ...
                pro = cr.fetchall()[0][0]
                cr.execute("insert into jm_zzu_plan_items (item,money,plan_con) values(%s, %s, %s)" % (pro, item, id))
                i += 1



class Zzu_Plan_items(models.Model):
    _name = 'jm.zzu.plan.items'
