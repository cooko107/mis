odoo.define('CustomManage.test', function (require) {"use strict";
...
var CompareListView = ListView.extend({
    ...
    generate_purchase_order: function () {
        var self = this;
        new Model(self.dataset.model).call("generate",[self.dataset.context.tender_id,self.dataset.context]).then(function(result) {
            self.ViewManager.action_manager.history_back();
        });
    },
});

core.view_registry.add('purchase_order_line_compare', CompareListView);
